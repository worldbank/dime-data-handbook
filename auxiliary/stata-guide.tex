...
despite the fact that they will spend a substantial share
of their professional time writing code.
Recent masters-level graduates that have joined our team
have tended to have very good theoretical knowledge,
but typically require a lot of training in practical coding skills.
To us, this is much like an architecture graduate who has learned
how to sketch, describe, and discuss
the concepts and requirements of a new building very well --
but who does not have the technical skills
to develop or contribute to actual technical blueprints
...
This appendix begins with a section containing instructions
...
no matter how proficient they are in Stata.
...
For example, DIME Analytics has developed two suites of Stata commands,
...
to standardize core data collection, management, and analysis workflows.
...
as providing some standardization for Stata code is also a goal of this team.
...
To see the code on GitHub, go to: \url{https://github.com/worldbank/dime-data-handbook/tree/master/code}.
If you are familiar with GitHub you can fork or clone the repository.
...
you will already have the data files used in the code.
...
That link offers a \texttt{.zip} file download
...
Each of our code examples and products is our best attempt
at accomplishing a specific technical task using ``good'' code.
Good code is more than simply clever code
or statistically correct code that produces the right results.
Good code it is useful and comprehensible to someone who hasn't seen it before
(or even yourself a few weeks, months or years later)
because it enables a new reader to determing what it is intending to do
and how it is doing it.
It is very easy to write code that produces correct results
but is completely impossible for another person to read or edit.
Many researchers have been trained to code in this way.
Therefore, when your code runs on your computer and you get the correct results,
you are only half-done writing \textit{good} code:
you then have to annotate and adapt it for someone else's understanding.
...
good structure means that it is easy to find which individual pieces of code
correspond to specific tasks and outputs,
both across and within each file and folder,
and including documentation such as \texttt{README} files.
Good structure also means that functional code blocks in each file
are sufficiently independent from each other
that they can be shuffled around, repurposed,
and even deleted without affecting other portions.
...
You should use common or built-in commands whenever possible
and build tasks using standard and explicit approaches
so that a reader can see exactly how work is build up.

\textbf{Style}, finally, is the way that the
non-functional elements of your code convey its purpose.
Elements like spacing, indentation,
and naming conventions (or lack thereof) can make your code much more
(or much less) accessible to someone who is reading it for the first time
and needs to understand it quickly and correctly.
We always tell staff to ``code as if a stranger would read it'',
and we implement code review processes so that a stranger does.

To do this well in Stata, you will have to get used to reading the help files.
...
that you get into the habit of constantly reading help files.
Whether you are new or not, you should frequently revisit help files for
the most common commands used by yourself and others --
...
In our code examples, we only use either Stata's built-in commands
or commands available from the SSC archive.
So, if your installation of Stata does not recognize a command in our code,
for example \texttt{randtreat}, then type \texttt{ssc install randtreat} in Stata.

Some commands on SSC are distributed in \textbf{packages}.
...
In other words, a style guide is an agreement on some components of good code.
Style guides therefore help improve the compatibility of the code
in that language that is produced by all programmers in a community.
It is through a shared style that newer programmers
can learn from more experienced programmers
