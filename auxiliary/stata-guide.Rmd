...
Commands on SSC are distributed in packages where multiple commands can be
included in a single package. This is the case, for example, of `iebaltab`,
which is part of the `ietoolkit` package. Therefore, it will not work to type
`ssc install iebaltab`. Instead, Stata will suggest using `findit iebaltab`,
which will search SSC (among other places) for a package containing a command
called `iebaltab.` Stata will find `iebaltab` in the package `ietoolkit` and
suggest installing it by typing `ssc install ietoolkit` in Stata instead.
...
the name in the syntax section at the top. Typically, only built-in commands have
support for abbreviations; user-written commands do not. Although Stata allows
some built-in commands to be abbreviated to one or two characters,
doing so can be confusing—two-letter
...
Furthermore, the back slash is an _escape character_ in Stata.
This means that if the following character is a special character with some
functionality, then that character special functionality is "escaped" and
read as is. Therefore, with back slashed, the file path ``"C:\Users\username\`proj'"``
will reference a folder called literally `` `proj' `` and not for a folder
with the name corresponding to the string stored in the local `proj`.
