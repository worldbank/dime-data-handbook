...
\VerbatimInput[frame=lines,numbers=left,label=randomization-program.do]{./code/randomization-program-1.do}

In the second part of the code, we set up for reproducibility,
then call the randomization program by name,
which applies precisely the randomization we programmed earlier.
The user-written \texttt{ritest} command\sidenote{\url{http://hesss.org/ritest.pdf}}
...
\VerbatimInput[frame=lines,numbers=left,label=simple-randomization.do]{./code/randomization-program-2.do}
