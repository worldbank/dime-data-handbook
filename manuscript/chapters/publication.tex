...
The main issue with using overleaf is that you need to upload inputs files
(such as figures and tables) manually to your browser.
...
So make sure to only move to Overleaf once there will not be substantive changes to results.
...
{\setstretch{0.7}
\VerbatimInput[frame=lines,numbers=left,label=sample.bib]
{./code/sample.bib}}

\noindent BibTeX citations are then used as follows:

{\setstretch{0.7}
\VerbatimInput[frame=lines,numbers=left,label=citation.tex]
{./code/citation.tex}}
...
{\setstretch{0.7}
\VerbatimInput[frame=lines,numbers=left,label=pandoc.sh]
{./code/pandoc.sh}}
...
\marginnote{See \url{https://github.com/worldbank/Water-When-It-Counts}
for an example of an organized directory structure.}
