...
JavaScript or Hadley Wickham's style guide for R.\sidenote{\url{http://adv-r.had.co.nz/Style.html}}
...
(texttt{*}, texttt{?}, and texttt{-}) is also discouraged,
...
prefer texttt{unab} or texttt{lookfor} to build that list in a local macro,
...
or (using texttt{camelCase}) \texttt{lastValue}, \texttt{allValues}, or \texttt{nValues}.
Nested locals (\texttt{if \`{}\`{}value\textquotesingle\textquotesingle})
...
This function should be used sparingly and commented extensively.
...
Therefore, it is faster to run outputs from commands like \texttt{reg} using the \texttt{qui} prefix.
