...
and ``\path{C:/users/username/git/project/...}'' on Windows.
...
``\path{/git/project/...}'' in the examples above, is added
to the user specific root path for each user,
``\path{/users/username}'' or ``\path{C:/users/username}'' in the examples above,
...
The first type of comment describes \textit{what} is being done.
...
The second type of comment explains \textit{why} the code is performing a task in a particular way.
...
data cleaning decision that make the published data differ from the original data 
to decision on how indicators are constructed.
...
Your donor or employer will most likely require you to hold a certification from a respected source.\sidenote{
...
is a very secure alternative to passwords when available.
...
they should use a VPN\sidenote{\textbf{Virtual Private Network (VPN):} Allows you to securely connect to a network you trust over an insecure connection. With the VPN you can securely communicate with other devices on the trusted network and make your traffic to the internet inaccessible to the host of the insecure connection.}
...
Third, all confidential data must be encrypted at all times.\sidenote{
	...
	\url{https://dimewiki.worldbank.org/Encryption}}\index{encryption}\index{data storage}
...
You can encrypt your data on disk (hard drive) level,
called \textbf{full disk encryption} (FDE), or on file level,
...
and require all members that handles confidential data to use it.

Most implementation of FDE use your computer login password to
...
Whenever FDE is available in your operative system
...
First, FDE is implemented differently in different operative systems
...
Since most FDE automatically decrypt your full disk each time you log in,
a malicious intruders that have gained access to your computer would have access to all your files.
Third and perhaps most important, 
FDE cannot be used when sharing confidential data 
over insecure channels like DropBox or email.
...
These encrypted folders protects your data 
when they are stored on your computer 
but can also be used to securely transfer data over 
insecure channels like DropBox and email.\index{data transfer}
...
that have gained access to your computer 
only gains access to the folders you have decrypted 
while your computer was compromised. 
Folders you rarely or never decrypt therefor 
remains protected even if someone gain access to your computer.
