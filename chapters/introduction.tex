...
An [empirical revolution]{\cite{angrist2017economic}}
...
Next, we turn to common research designs for \textbf{causal inference}{\sidenote{causal inference: identifying the change in outcome \textit{caused} by a particular intervention}}, and consider their implications for data structure. 
...
(we offer analogous examples in R as much as possible). 
In the book, code examples will be presented like the following:

\codeexample{code.do}{./code/code.do}

We ensure that each code block runs independently, is well-formatted, and uses built-in functions as much as possible.
