...
**dynamic documents** are a way to simplify writing workflows:
...
<!-- keeping it simple -->
While LaTeX *can* produce complex formatting,
...
Specifically, LaTeX inserts references in text using the `cite` command.\index{bibliography management}
...
using utilities such as `pandoc`.^[https://pandoc.org]
...
The CSL (Citation Styles Library) file^[
...
a cleaned dataset and supporting documentation ready.
...
with only the published clean data in hand.^[
  For an example, see the replication package for @kondylis2020demand
  in https://github.com/worldbank/rio-safe-space.
]
...
you should carefully perform a **final de-identification**.
...
indirect identifiers, and assess the statistical disclosure risk of your data.^[
  **Disclosure risk:** the likelihood that a released data record can be associated with an individual or
  organization.
...
approximately 87% of the US population.^[@Sweeney2000]
...
At this stage, the `sdcMicro` package,^[
  @benschop2019statistical
]
...
embargo specific variables that are required for the analysis but cannot be released publicly.
...
and a formal citation for your data, so you can reference it in other research outputs.^[https://www.doi.org]
...
*Licensed access* data is restricted to users
...
or some other type of material,^[
  Such as https://github.com/worldbank/sdgatlas2018,
  https://blogs.worldbank.org/opendata/how-we-mass-produced-reproducible-human-capital-project-country-briefs, 
  or https://blogs.worldbank.org/impactevaluations/what-development-economists-talk-about-when-they-talk-about-reproducibility
]
...
is often referred to as a **reproducibility package**.\index{reproducibility package}
...
including selecting a specific version of the package if necessary^[
  The `renv` package helps you maintain reproducibility by tracking versions of packages used:
...
![*Demand for Safe Spaces* GitHub landing page](examples/home.png){width=46%}
![*Demand for Safe Spaces* reproducibility package](examples/github.png){width=53%}
...
```{r, fig.cap = "Publication tasks and outputs", echo = FALSE}
knitr::include_graphics('img/Publication.png')
```
