...
Although data quality assurance and data tidying are often done at the same time,
particularly one it comes to guaranteeing that observations are uniquely identified,
we will treat it as the first step in data cleaning.
This is because quality assurance can only be finalized using tidied data.
In practice, both tidying and quality monitoring should be done continuously as data is received.
...
you are receiving data from a partner,
...
It is also possible to automate communication of errors to data providers
...
connectivity (particularly for survey teams)
and coding skills of the team preparing the HFC workflows.
...
unless everyone accessing it is always working in a secure environment,
which is rarely the case.
In most cases,
it will be necessary to keep at least a subset of the data 
stored in an encrypted format through the data analysis process.
This data will need to be decrypted and loaded to a \textit{secure environment}
every time you run code to analyzed or process it.
