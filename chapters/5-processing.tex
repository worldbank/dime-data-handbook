...
imported from survey platforms in this format,
which is called \textbf{wide format}.\sidenote{\textbf{Wide data:}
	a data table where a single variable is divided into multiple columns,
	for example one for each individual in a household.
}
Wide data could have, for instance,
one column for the household variable called \texttt{ownsfridge},
but a few columns for the household member variables \texttt{sex\_1}, \texttt{sex\_2}, etc.
...
and household-member variables are reshaped to member level
and stored in another tidy data table,
...
The household members data will be uniquely identified by
...
In both examples above, your dataset is made up of two data tables,
...
There must also be a clear way to connect each
tidy data table to a master dataset,
and thereby also to all other datasets.
This is done by deciding which data table is the main data table,
and that data table's unit of observation is
...
the household data table would be the main table.
