...
in a \textbf{tabular dataset}\sidenote{\textbf{Tabular data:}
...
It is also possible for a raw dataset to not include a variable that uniquely identifies all observations, 
...
occurrences of duplicated entries for a dataset's unique identifier using
...
So it would have, for instance, one column called \texttt{ownsfridge},
...
while a household member ID will be this tables' unique identifier.
Note that such dataset is only appropriate for analysis at the person level.
