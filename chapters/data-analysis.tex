...
Though it is best practice to be transparent about different
specifications tried and tests performed,
only a few of them will be considered ``main results''.
...
Independently of your store decisions,
you will need to update your outputs frequently.
And if you have tried to recreate a result after a few months,
you probably know that it can be hard to remember where the code that created it was save.
Naming conventions and code organization play a key role
in not re-writing scripts again and again.
It is common for teams to maintain one analysis file or
folder with draft code or exploratory analysis.
These contain pieces of code that are stored for reference purposes,
but not cleaned up to be included in any final outputs yet.
Once you are satisfied that your output presents a result in the clearest manner possible,
it should be renamed and moved to ``final analysis'' folder.
...
% File formats
The final step to create research products for public consumption is to
...
The final section of this chapter will discuss a few different options of software
that can be useful to do this. 
...
which are ready to be included in a paper or report.
...
You think that it's not worth coding a small formatting adjustment,
...
However, don't spend too much time formatting tables and graphs until 
you have come to a decision about which of them will be used for your final product.\sidenote{
...
Our team has created a few products to automate the creation of common outputs 
and save you precious research time.
...
included in your code and comments that are not otherwise visiable in the output.
Examples of this include such as sample use, 
...
so the cost of entry is relatively lower.
...
to papers can be more easily be formatted into their specific layout. 
	
	
\subsection{Documenting exploratory analysis}	

% Markdown
If all team members working on a dynamic document are comfortable using a statistical software,
built-in dynamic document engines are also a good option.
This means you can write both text (in Markdown\sidenote{\url{https://www.markdownguide.org/}}) and code in the script,
...
These type of dynamic document tools is typically best used by the team members that work more closely with code,
...
but include extensive collaboration features, 
