...
Loading encrypted data frequently can be disruptive to the work flow.
...
% Data description ------------------------------------------------------------------
Note that if you are using secondary data, the tasks described above can likely be skipped.
...
First, we suggest keeping the same variable names as in the survey instrument, so it's straightforward to link data points for a variable to the question that originated them.
...
It subtly encourages poor practices such as not clearing the workspace and not loading fresh data for each analysis task. 
