...
and use the same filepaths, shortcuts, and macro references.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/DataWork\_Folder}}
...
At the top level of the structure created by \texttt{iefolder} are what we call survey round folders.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/DataWork\_Survey\_Round}}
...
The raw data will invariably come in a host of file formats and these files
...
encrypted\sidenote{\url{https://dimewiki.worldbank.org/wiki/Encryption}} and be
...
The raw data sets should never be edited directly. This is true even in the
rare case when the raw data cannot be opened due to incorrect encoding and
non-English character is causing rows or columns to break at the wrong place
when the data is imported. In this scenario, you should create a copy of
the raw data where you manually remove the special characters and securely back
up \textit{both} the broken and the fixed copy of the raw data. You will only
keep working from fixed copy, but you keep both copies in case you later
realize that the manual fix was done incorrectly.
...
at this stage, means stripping the data set of direct identifiers.\sidenote{\url{ https://www.povertyactionlab.org/sites/default/files/resources/J-PAL-guide-to-deidentifying-data.pdf}}
...
as well as allowing for more sophisticated disclosure risk calculations.\sidenote{
	\url{http://sdctools.github.io/sdcMicro/articles/sdcMicro.html}}
...
where you can easily select which variables to keep or drop.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/Iecodebook}}
...
Data cleaning is the second stage in the transformation of data you received from the field into data that you can analyze.\sidenote{\url{https://dimewiki.worldbank.org/wiki/Data\_Cleaning}}
...
such as renaming, relabeling, and value labeling, much easier.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/iecodebook}}
\index{iecodebook}
...
Variable and value labels should be accurate and concise.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/Data\_Cleaning\#Applying\_Labels}}
...
These materials are essential for data documentation.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/Data\_Documentation}}
\index{Documentation}
...
(one item in the bundle) in the survey to the unit of analysis (the household).\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/Unit\_of\_Observation}}
...
\texttt{iebaltab}\sidenote{\url{https://dimewiki.worldbank.org/wiki/Iebaltab}}
creates and exports balance tables to excel or {\LaTeX}.
\texttt{ieddtab}\sidenote{\url{https://dimewiki.worldbank.org/wiki/Ieddtab}}
...
There are numerous commands to export outputs from both R and Stata to a myriad of formats.\sidenote{
	Some examples are \href{ http://repec.sowi.unibe.ch/stata/estout/}{\texttt{estout}}, \href{https://www.princeton.edu/~otorres/Outreg2.pdf}{\texttt{outreg2}},
...
unit of observation, unit of measurement and variable definition.\sidenote{
	\url{https://dimewiki.worldbank.org/wiki/Checklist:\_Reviewing\_Graphs} \\
	\url{https://dimewiki.worldbank.org/wiki/Checklist:\_Submit\_Table}}
