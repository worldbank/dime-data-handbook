...
pays off substantial dividends throughout the process.
...
The focus in version control is often code, but changes to analysis output should, when possible, be version controlled together with the edit to the code that caused the change.
This way you know what edit in the code led to what edit in the analysis.
...
which should contain only information that are received directly from the field.
...
If the answer to either of two questions above is yes,
...
Data cleaning is the second stage in the transformation of data you received from 
the field into data that you can analyze.\sidenote{\url{
https://dimewiki.worldbank.org/wiki/Data\_Cleaning}}
...
so that level of the data set goes from the unit of observation (one item in the bundle) 
in the survey to the unit of analysis (the household).\sidenote{
\url{https://dimewiki.worldbank.org/wiki/Unit\_of\_Observation}}
