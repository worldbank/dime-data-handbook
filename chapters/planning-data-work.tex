...
Ensure you know how to get the \textbf{absolute filepath} for any given file.
Using the absolute filepath, starting from the filesystem root,
...
We will write filepaths such as \path{/Dropbox/project-titleDataWorkEncryptedData/},
assuming the ``Dropbox'' folder lives inside your home folder.
Filepaths will use forward slashes (\texttt{/}) to indicate folders,
and typically use only A-Z (the 26 English characters),
dashes (\texttt{-}), and underscores (\texttt{\_}) in folder names and filenames.
You should \textit{always} use forward slashes (\texttt{/}) in filepaths in code,
...
but in general, there are three file sharing paradigms that are the most common.
...
that you are going to be doing, and plan which types of files
...
Email is, simply put, not a system. It is not a system for anything. Neither is WhatsApp.
  \index{email} \index{WhatsApp}
These tools are developed for communicating ``now'' and this is what they does well.
These tools are not structured to manage group membership or to present the same information
...
One popular and free implementation of this system is the one found in GitHub project boards.
...
as a part of our \texttt{ietoolkit} package.
  \index{\texttt{iefolder}} \index{\texttt{ietoolkit}}
...
reserving the ``project folder'' for data collection and other management work.
...
and non-technical files are usually \textbf{binary} files.
  \index{plaintext} \index{binary files}
...
tools such as {\LaTeX} and dynamic documents.
  \index{{\LaTeX}} \index{dynamic documents}
...
Because of the high degree with which code files depend on file structure,
...
For technical files, however, a more detailed version control system is usually desirable.
...
Technical files have stricter requirements than non-technical ones.\sidenote{
...
Finally, agree with your team on a plan to review code as it is written.
...
is also the easiest way to ensure a smooth project handover.

% ----------------------------------------------------------------------------------------------
\subsection{Output management}

The final task that needs to be discussed with your team is the best way to manage outputs.
...
Raw outputs in formats like \texttt{.tex} and \texttt{.eps} can be managed with Git,
while final outputs like PDF, PowerPoint, or Word, can be kept in a synced folder.
Storing plaintext outputs on Git makes it easier to identify changes that affect results.
...
is simply to commit all your code and outputs,
...
researchers are increasinly choosing to prepare even final outputs
...
It is common for teams to maintain an analyisis file or folder with ``exploratory analysis'',
which are pieces of code that are commented and written only to be found again in the future,
but not cleaned up to be included in any outputs yet.
Once you are happy with a partiular result or output, however,
...
so that before the line that runs a particular analysis script there are a few lines of comments listing
