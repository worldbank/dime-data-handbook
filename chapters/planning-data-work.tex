...
All machines should have their hard disks encrypted;
this should be built-in to most modern operating systems
(the service is currently called BitLocker on Windows or FileVault on MacOS).
Encryption prevents your contents from ever being accessed without the password.
Then, make sure your computer is backed up to prevent information loss.
Follow the \textbf{3-2-1 rule}:
have 3 copies of everything, on at least
2 different hardware devices you have access to,
with 1 offsite storage.\sidenote{
...
(or a secondary computer),
and a remote copy managed by a cloud backup service.
...
using forward slashes, and mostly use only A-Z, dash (\texttt{-}), and underscore (\texttt{_}).
