...
% Chapter overview
In this chapter, we discuss the necessary step to transform
cleaned raw data into informative analysis outputs such as tables and figures.
The suggested workflow starts, thus, where the last chapter ended:
...
and incorporate any corrections needed.\sidenote{
	See \autoref{ch:5} 5 for discussions on how to 
...
prevents such problem and ensures consistency across different outputs.
...
research designs comprising different units of observation and analysis.
...
To do so, you will start from the constructed dataset at student level;
average the test scores of all students taught by the same teacher;
and merge this aggregate measure to the teacher-level dataset.
...
that should be documented in the \textbf{data linkage table};
...
or similar data collected from different contexts.\index{iefieldkit}\index{iecodebook}\sidenote{
...
the solution to such problems is usually a single well-written google search away,
...
the you need them, the more reproducible your output is.
...
so at least this file can be updated when the code changes.
Finally, you can export tables to csv files,
and have a separate Excel file containing the formatting that is linked to the csv.
In our experience, this setup is very unstable,
and requires a point person in the team to be checking it continuously,
but it is a step in the right direction compared to simply copying and pasting.
