...
a particular hypothesis will be approached; only that it will be.\sidenote{\url{https://dimewiki.worldbank.org/wiki/Pre-Registration}}
...
to the level of a formal publication.\sidenote{
  \url{https://dimewiki.worldbank.org/wiki/Registered_Reports}}
...
Major publishers and funders have taken steps to require that code and data
are accurately reported, cited, and preserved as outputs in themselves.\cite{vilhuber2020report}
Most notably, the American Economic Association now requires
the deposit of all raw data, all cleaning code, all analysis data,
and all analytical code prior to acceptance of any work.
Common standards imposed from other journals and funders feature both ex ante
(``regulation'') and ex post (``verification'') policies.\cite{stodden2013toward}
...
at least from analytical code and data,
and we require the publication of raw data.
...
such that the final package produces exactly the same results as in the paper.
The team checks whether the package includes sufficient documentation
to reproduce the code, and provides advice on improving the
clarity and quality of these outputs.
...
Making research reproducible is also a public good.
