...
particularly for projects using raw data.
...
% Intro
The very first step in creating analysis-friendly data sets
...
Each element or cell in this table is a \textbf{data point}\sidenote{\textbf{Data point:}
	a value representing a measure of an attribute for one individual unit.}, that is,
...
In the example of a household roster,
...
In a household roster, the unit of observation is a person.
The collection of all data points for each individual unit of observation is an 
...
about the parties involved in each transaction on the same table.
The next two sections will outline steps to check if your data is tidy,
and how to tidy it if it is not.
...
The reshaped dataset will have a new variable indicating the member ID,
...
% Generalizing
Alternatively, if each row in the untidy dataset represents the lower level of observation,
