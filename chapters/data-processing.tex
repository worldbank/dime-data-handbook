...
development data is most commonly transferred as one or multiple tables.\sidenote{\textbf{Table:}
...
that makes interacting with the easiest.
When applied to statistics, we call data in such format \textbf{tidy}.\sidenote{\textbf{Tidy data:}
...
but raw survey datasets are usually messy in a similar way.
...
just suppose you need to calculate the average age of the respondents in your sample.
The first thing you will need to do is change the format of your table so that each row represents one individual.
Tidy data sets, on the other hand, 
...
which may not be consistent with your design.
