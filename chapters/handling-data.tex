...
Raw data which contains PII \textit{must} therefore be \index{encryption}\textbf{encrypted}\sidenote{
...
Proper encryption means that, even if the files were to be intercepted my a malicious 
``intruder'' or accidentally made public, the information that would be leaked would 
be completely unreadable and unusable. Proper encryption also means that no one not 
listed on the IRB may have access to the decryption key, which means that it usually not 
...
that is, to remove direct identifiers of the individuals in the dataset.\sidenote{
...
can be \textit{within the research team} without the need for encryption.
